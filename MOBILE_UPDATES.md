...
Заполняет супер-владелец в панели; клиент только **читает**. В имени получателя
(收货人) уже вшит **код текущего клиента** — по нему в Китае опознают коробку.
...
```json
{
  ...
  "postal_code": "510000",
  "instructions": "Обязательно оставьте код в имени получателя, иначе посылку не опознают.",
  "is_active": true,
  "region": "广东省广州市白云区",
  "recipient": "C1234567",
  "one_line": "C1234567 +8613800138000 广东省广州市白云区 XX路 100号 315CARGO仓库 510000",
...
Поля:
- `recipient` — **收货人 = код клиента** (подставляется автоматически под текущего клиента). Именно это идёт в имя получателя в PDD.
- `one_line` — **готовая строка для вставки** в PDD (умное распознавание / 智能填写): `收货人 телефон 省市区 детальный_адрес индекс`. Код клиента уже внутри.
...
- `recipient_name` — базовое имя-запас (обычно пустое; для клиента используйте `recipient`, а не его).
...
4. Показать поля + `instructions`. Дать кнопку **«Скопировать»**:
   ...
   - опционально — копировать поля по отдельности (收货人=`recipient`, телефон, регион, адрес).
5. Подсветить, что **收货人 = его код** и его нельзя убирать.
...
class DeliveryAddress {
  final String recipient;       // 收货人 = код клиента
