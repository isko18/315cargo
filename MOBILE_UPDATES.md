...
> 🆕 **2026-08-09 (1688):** третья площадка на том же контракте —
> `/api/integrations/1688/{connect,disconnect,status,ingest,session-expired}/`.
> Подключения всех трёх независимы. 1688 на том же движке Alibaba, что Taobao
> (`h5api.m.1688.com`, `mtop.alibaba.cbu.wireless.uniform.render.*`), поэтому
> разбор общий: шлите ответ целиком. Точное имя API списка заказов и раскладку
> полей нужно сверить на реальном ответе — детали в
> [FLUTTER_1688.md](FLUTTER_1688.md). Фильтр посылок: `?source=1688`.

> 🆕 **2026-08-08 (маркетплейс у посылки + все статусы):** у посылки появились
> **`source`** (`pinduoduo` / `taobao` / `1688` / `manual`) и `source_display_name` —
> по ним делается фильтр «откуда посылка». Фильтры API: `?source=taobao`,
> `?source_in=taobao,pinduoduo`; без фильтра приходят все. Посылка со сканера
> (без заказа) — `manual`, из списка не пропадает.
>
> Заказы больше **не выбрасываются**: раньше отменённые и неоплаченные молча не
> сохранялись, и клиент их в приложении не видел. Теперь сохраняются все, со
> своим статусом — `待付款` → «Оформлен», `交易关闭`/`已退款` → «Отменён».
> Посылка при этом заводится только под то, что реально поедет: у отменённого и
> неоплаченного заказа посылки нет.

> 🆕 **2026-08-08 (Taobao):** подключён по той же схеме, что и Pinduoduo —
> `/api/integrations/taobao/{connect,disconnect,status,ingest,session-expired}/`.
> Подключения независимы: клиент может быть подключён к обоим сразу. Перехватывать
> нужно ответ mtop по подстроке `queryboughtlist`, признак логина — куки
> `_tb_token_` + `cookie2`. Сырые заказы шлите как есть, разбор на сервере.
> В ответах интеграции появилось поле `marketplace`. Детали —
> [FLUTTER_TAOBAO.md](FLUTTER_TAOBAO.md).

> ⛔ **Изменено 2026-08-06 (адрес: убран индекс + приписка карго):** `postal_code`
> **удалён из ответа** `/api/delivery-address/` и больше не входит в `one_line` —
> маркетплейс подставляет индекс сам по 省市区, а лишнее число в конце сбивало
> распознавание. Уберите поле из экрана адреса. Появилось **`detail_address_full`** —
> детальный адрес вместе с припиской карго (у каждого карго своя: `…仓315库` + `东`).
> Для ручного заполнения полей PDD используйте его, а не `detail_address`.
> Новый порядок в `one_line`: `ФИО телефон 省市区 детальный_адрес+приписка КОД_КАРГО КОД_КЛИЕНТА`.

> ✅ **2026-08-06 (пуши включены по-настоящему):** service-account `cargo-dc9d3` стоит
> на проде, отправка идёт через FCM HTTP v1. Сообщение собирается по вашему ТЗ:
> `notification` + `data` (только строки, `type` всегда), `android.priority=high`,
> `channel_id=cargo315_default`, `apns-priority=10`, бейдж = число непрочитанных.
> Добавлен **`DELETE /api/device-tokens/`** с телом `{"token": "..."}` → `204` —
> подключайте в `PushService.onLoggedOut()`. Мёртвые токены гасятся только на
> `UNREGISTERED` / `SENDER_ID_MISMATCH` / `INVALID_ARGUMENT` / `NOT_FOUND`.
> Осталось на вашей стороне: APNs-ключ в Firebase Console и капабилити в Xcode.
...
```json
{
  ...
  "region": "广东省广州市白云区",
  "recipient": "张伟",
  "one_line": "程先生 13250150777 广东佛山南海 里水镇和顺鹤峰1号仓315库东 x69610 ISI-0002",
...
Поля:
- `recipient` — **收货人 = ФИО получателя** на складе. Именно это идёт в имя получателя в PDD. Берётся из карго клиента (у каждого карго свой человек на приёмке), фолбэк — общее ФИО, затем код клиента.
- `one_line` — **готовая строка для вставки** в PDD (умное распознавание / 智能填写): `收货人 телефон 省市区 детальный_адрес+приписка КОД_КАРГО КОД_КЛИЕНТА`. Оба кода уже внутри, индекса нет.
...
- `phone`, `province`, `city`, `district`, `detail_address_full` — по отдельности (для ручного заполнения полей PDD). Индекс не нужен.
...
### Обновление маршрута (сентябрь 2026) — нужен ещё один статус

Добавлен статус **`customs` — «На таможне»**. Внесите его в `ParcelStatus` и в
`parseParcelStatus` (`'customs' => ParcelStatus.customs`), в таймлайне ставьте
между `in_transit` и `at_pickup_point`. Если fallback `_ => …` у вас уже есть,
экран не упадёт, но статус будет отображаться неверно.

Сама цепочка сокращена и теперь выглядит так:

```
Прибыл на склад в Китае
   +1 день  → В пути        (in_transit)
   +7 дней  → На таможне    (customs)
   дальше — только реальный скан оператора
```

Что изменилось по существу: **таймер больше не объявляет прибытие**. Раньше
`arrived_kyrgyzstan` проставлялся автоматически на 9-й день, и пуш «Посылка
прибыла в Кыргызстан» уходил клиенту независимо от того, где посылка на самом
деле. По данным прода реальное время в пути — от 0.3 до 16 дней, поэтому
уведомление было ложным у большинства. Теперь `arrived_kyrgyzstan`,
`at_pickup_point` и всё, что дальше, приходят только от скана.

`processing` из авто-цепочки убран. Статус остаётся в списке (старые посылки в
нём есть), но новые в него больше не попадают.
