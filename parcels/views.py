...
class ParcelPagination(LimitOffsetPagination):
    """Постраничный список посылок — по запросу, а не всегда.

    default_limit = None принципиально. С ним страница включается только
    когда клиент прислал ?limit=: без параметра ответ остаётся голым
    массивом, как был до появления пагинации. Иначе смена формы ответа на
    {count, next, previous, results} ломает уже установленные мобильные
    приложения — обновить их одновременно с бэкендом нельзя.
